 ...
 *县<select id="Area" name="Area"></select>  
 *  
 *JS代码
 *androidWheelInit('Province', 'City', 'Area');  //将省市县select的ID传进来
 *
 **/
var androidWheelInit = function(myProvince, myCity, myArea, defaultProvince, defaultCity, defaultArea)
{
    var Province = document.getElementById(myProvince);
    var City = document.getElementById(myCity);
    var Area = document.getElementById(myArea);

    function mySelect(site, str)
    {
        for(var i=0; i<site.options.length; i++)
        {
            if(site.options[i].value == str)
            {
    ...
    function addOption(site, str, obj)
    {
    ...
    {
        addOption(Province, ProvinceList[i].name, ProvinceList[i]);
    }
    mySelect(Province, defaultProvince);
    changeProvince();
